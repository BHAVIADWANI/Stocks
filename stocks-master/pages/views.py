...
def news_data(request):
    ...
    try:
        if stock_name == "reliance":
            with open('pages/static/json_data/reliance.json', 'r') as file:
                content = json.load(file)
        elif stock_name == "axis":
            with open('pages/static/json_data/axis.json', 'r') as file:
                content = json.load(file)
        elif stock_name == "hdfc":
            with open('pages/static/json_data/hdfc.json', 'r') as file:
                content = json.load(file)
    except FileNotFoundError:
        print("Error: File not found.")
    except IOError:
        print("Error: Input/Output error occurred while processing the file.")
    except Exception as e:
        print("Error:", str(e))
    return JsonResponse({'news': content})
...
class ChartView(LoginRequiredMixin, View):
    ...
    def get(self, request) -> JsonResponse | HttpResponse:
        ...
        if (
            ...
            try:
                ...
                raise Http404(
                    f"Error encountered while fetching data from live market feed: {instrument_key}"
                )
            ...
            url = f'https://api.upstox.com/v2/historical-candle/{instrument_key}/1minute/2024-05-17/2024-05-17'
    ...
    def live_market_feed(self, instrument_key="NSE_EQ%7CINE002A01018"):
        ...
        access_token = 'eyJ0eXAiOiJKV1QiLCJrZXlfaWQiOiJza192MS4wIiwiYWxnIjoiSFMyNTYifQ.eyJzdWIiOiI3NUFDMkIiLCJqdGkiOiI2NjQ0NDU2OGQ2YWU4ZDI0ZGNjMmM3Y2MiLCJpc011bHRpQ2xpZW50IjpmYWxzZSwiaWF0IjoxNzE1NzUwMjQ4LCJpc3MiOiJ1ZGFwaS1nYXRld2F5LXNlcnZpY2UiLCJleHAiOjE3MTU4MTA0MDB9.PHDXWu8EideoOSSjBWDqW6PsDlxjxkaL8gqyZ9un8DE'
...
def trade_list(request):
    ...
    for stock_name in stocks:
        bought_quantity = sum(trade.quantity for trade in trades if trade.stock_name == stock_name and trade.is_buy)
        sold_quantity = sum( trade.quantity for trade in trades if trade.stock_name == stock_name and not trade.is_buy )
        remaining_quantity = bought_quantity - sold_quantity
        if remaining_quantity > 0:
            stock_summary.append( {"stock_name": stock_name, "remaining_quantity": remaining_quantity} )
